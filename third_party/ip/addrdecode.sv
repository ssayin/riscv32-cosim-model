...
module addrdecode #(
    ...
    parameter logic [NS*AW-1:0] SLAVE_MASK = (NS <= 1) ?
        0 : {{(NS - 2) {3'b111, {(AW - 3) {1'b0}}}}, {(2) {4'b1111, {(AW - 4) {1'b0}}}}},
    //
    ...
    // write to it.
    parameter logic [NS-1:0] ACCESS_ALLOWED = -1,
    //
    // If OPT_REGISTERED is set, address decoding will take an extra
    // clock, and will register the results of the decoding
    // operation.
    parameter logic [0:0] OPT_REGISTERED = 0,
    //
  ...
  localparam logic [0:0] OPT_NONESEL = (!ACCESS_ALLOWED[0]) || (SLAVE_MASK[AW-1:0] != 0);
  //
  wire [  NS:0] request;
  ...
  // {{{
  always_comb
    for (int iM = 0; iM < NS; iM = iM + 1)
      prerequest[iM] = (((i_addr ^ SLAVE_ADDR[iM*AW+:AW]) & SLAVE_MASK[iM*AW+:AW]) ==
                        0) && (ACCESS_ALLOWED[iM]);
  // }}}
  ...
  generate
    if (OPT_NONESEL) begin : g_no_default_request
      ...
      //
      always_comb begin
        for (int iM = 0; iM < NS; iM = iM + 1) r_request[iM] = i_valid && prerequest[iM];
  ...
  // {{{
  generate
    if (OPT_NONESEL) begin : g_nonesel
  ...
  // {{{
  generate
    if (OPT_REGISTERED) begin : g_registered
