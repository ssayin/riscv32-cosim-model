...
module axixbar #(
  ...
  wire [NMFULL-1:0] write_qos_lockout, read_qos_lockout;
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : W1_DECODE_WRITE_REQUEST
      ...
      // awskid, the skidbuf for the incoming AW* channel
      // {{{
      skidbuf #(
          // {{{
      ...
      skidbuf #(
        ...
        S_AXI_WVALID[N],
        S_AXI_WREADY[N],
        {S_AXI_WDATA[N*DW+:DW], S_AXI_WSTRB[N*DW/8+:DW/8], S_AXI_WLAST[N]},
        m_wvalid[N],
        slave_waccepts[N],
      ...
      always @(*) begin
        ...
        // pending.
        if (!mwgrant[N]) slave_awaccepts[N] = 1'b0;
        if (write_qos_lockout[N]) slave_awaccepts[N] = 1'b0;
        if (mwfull[N]) slave_awaccepts[N] = 1'b0;
        // Don't accept a packet unless its to the same slave
        // the grant is issued for
        if (!wrequest[N][mwindex[N]]) slave_awaccepts[N] = 1'b0;
        if (!wgrant[N][NS]) begin
          if (!slave_awready[mwindex[N]]) slave_awaccepts[N] = 1'b0;
        end else if (berr_valid[N] && !bskd_ready[N]) begin
      ...
      always @(*) begin
        ...
        if (!mwgrant[N]) slave_waccepts[N] = 1'b0;
        if (!wdata_expected[N] && (!OPT_AWW || !slave_awaccepts[N]))
          slave_waccepts[N] = 1'b0;
        if (!wgrant[N][NS]) begin
          if (!slave_wready[mwindex[N]]) slave_waccepts[N] = 1'b0;
        end else if (berr_valid[N] && !bskd_ready[N]) slave_waccepts[N] = 1'b0;
      end
      // }}}
      ...
      always @(*) begin
        ...
        wrequest[N] = 0;
        if (!mwfull[N]) wrequest[N][NS:0] = wdecode;
      end
      ...
      // {{{
      if (!OPT_QOS || NM == 1) begin : WRITE_NO_QOS
        ...
        always @(posedge S_AXI_ACLK)
          if (!S_AXI_ARESETN) r_write_qos_lockout <= 0;
          else begin
            r_write_qos_lockout <= 0;

            for (iN = 0; iN < NM; iN = iN + 1)
            if (iN != N) begin
              if (m_awvalid[N] && (|(wrequest[iN][NS-1:0] & wdecode[NS-1:0])) &&
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : R1_DECODE_READ_REQUEST
      // {{{
      reg         r_arvalid;
      wire [NS:0] rdecode;
      ...
      always @(*) begin
        ...
        rrequest[N] = 0;
        if (!mrfull[N]) rrequest[N][NS:0] = rdecode;
      end
      ...
      always @(*) begin
        ...
        if (!mrgrant[N]) slave_raccepts[N] = 1'b0;
        if (read_qos_lockout[N]) slave_raccepts[N] = 1'b0;
        if (mrfull[N]) slave_raccepts[N] = 1'b0;
        // If we aren't requesting access to the channel we've
        // been granted access to, then we can't accept this
        // verilator lint_off  WIDTH
        if (!rrequest[N][mrindex[N]]) slave_raccepts[N] = 1'b0;
        // verilator lint_on  WIDTH
        if (!rgrant[N][NS]) begin
          if (!slave_arready[mrindex[N]]) slave_raccepts[N] = 1'b0;
      ...
      if (!OPT_QOS || NM == 1) begin : READ_NO_QOS
        ...
        always @(posedge S_AXI_ACLK)
          if (!S_AXI_ARESETN) r_read_qos_lockout <= 0;
          else begin
            ...
            if (iN != N) begin
              if (m_arvalid[iN] &&
                  !slave_raccepts[N] && (|(rrequest[iN][NS-1:0] & rdecode[NS-1:0])) &&
  ...
  always @(*) begin : W2_DECONFLICT_WRITE_REQUESTS
    ...
    for (iN = 0; iN <= NM; iN = iN + 1) wrequested[iN] = 0;
  ...
  always @(*) begin : R2_DECONFLICT_READ_REQUESTS
    ...
    for (iN = 0; iN < NM; iN = iN + 1) rrequested[iN] = 0;
    ...
    for (iM = 0; iM < NS; iM = iM + 1) begin
      ...
      for (iN = 1; iN < NM; iN = iN + 1) begin
        // Continue to request any channel with
        // a grant and pending operations
        if (rrequest[iN-1][iM] && rgrant[iN-1][iM]) rrequested[iN][iM] = 1;
        if (rrequest[iN-1][iM] && (!mrgrant[iN-1] || mrempty[iN-1]))
          rrequested[iN][iM] = 1;
        // Otherwise, if it's already claimed, then
        // it can't be claimed again
        if (rrequested[iN-1][iM]) rrequested[iN][iM] = 1;
      end
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : W3_ARBITRATE_WRITE_REQUESTS
      ...
      always @(*) begin
        ...
        if (berr_valid[N]) stay_on_channel = 1;
      end
      // }}}
      ...
      always @(*) begin
        ...
        // has requested it
        requested_channel_is_available =
            |(wrequest[N][NS-1:0] & ~swgrant & ~wrequested[N][NS-1:0]);

        // Of course, the error pseudo-channel is *always*
        // available to us.
        if (wrequest[N][NS]) requested_channel_is_available = 1;

        // Likewise, if we are the only master, then the
        // channel is always available on any request
        if (NM < 2) requested_channel_is_available = m_awvalid[N];
      end
      // }}}
      ...
      if (OPT_LINGER == 0) begin
        ...
        always @(posedge S_AXI_ACLK)
          if (!S_AXI_ARESETN || wgrant[N][NS]) begin
            r_linger       <= 0;
            linger_counter <= 0;
          end else if (!mwempty[N] || bskd_valid[N]) begin
      ...
      always @(*) begin
        leave_channel = 0;
        if (!m_awvalid[N] && (!linger || wrequested[NM][mwindex[N]]))
      ...
      always @(wrequest[N]) begin
        ...
        if (wrequest[N][iM]) requested_index = requested_index | iM[LGNS-1:0];
      end
    ...
    for (N = NM; N < NMFULL; N = N + 1) begin : g_mwindex
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : R3_ARBITRATE_READ_REQUESTS
      ...
      always @(*) begin
        ...
        if (read_qos_lockout[N]) stay_on_channel = 0;

        // We must stay on this channel until we've received
        // our last acknowledgment signal.  Only then can we
        // switch grants
        if (mrgrant[N] && !mrempty[N]) stay_on_channel = 1;
        ...
        if (rgrant[N][NS] && (!rerr_none[N] || rskd_valid[N])) stay_on_channel = 1;
      end
      // }}}
      ...
      always @(*) begin
        ...
        // has requested it
        requested_channel_is_available =
            |(rrequest[N][NS-1:0] & ~srgrant & ~rrequested[N][NS-1:0]);

        // Of course, the error pseudo-channel is *always*
        // available to us.
        if (rrequest[N][NS]) requested_channel_is_available = 1;

        // Likewise, if we are the only master, then the
        // channel is always available on any request
        if (NM < 2) requested_channel_is_available = m_arvalid[N];
      end
      // }}}
      ...
      always @(*) begin
        leave_channel = 0;
        if (!m_arvalid[N] && (!linger || rrequested[NM][mrindex[N]]))
          // Leave the channel after OPT_LINGER counts
          // of the channel being idle, or when someone
          // else asks for the channel
          leave_channel = 1;
        if (m_arvalid[N] && !rrequest[N][mrindex[N]])
          ...
          // to any other channel
          leave_channel = 1;
        if (read_qos_lockout[N]) leave_channel = 1;
      end
      // }}}
      ...
      always @(rrequest[N]) begin
        requested_index = 0;
        for (iM = 0; iM <= NS; iM = iM + 1)
        if (rrequest[N][iM]) requested_index = requested_index | iM[LGNS-1:0];
      end
    ...
    for (N = NM; N < NMFULL; N = N + 1) begin : g_mrindex
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin : W4_SLAVE_WRITE_INDEX
      ...
      if (NM <= 1) begin
        ...
        always @(*) begin
          reqwindex = 0;
          for (iN = 0; iN < NM; iN = iN + 1)
          if ((!mwgrant[iN] || mwempty[iN]) && (wrequest[iN][M] && !wrequested[iN][M]))
    ...
    for (M = NS; M < NSFULL; M = M + 1) begin : g_swindex
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin : R4_SLAVE_READ_INDEX
      ...
      if (NM <= 1) begin
        ...
        always @(*) begin
          reqrindex = 0;
          for (iN = 0; iN < NM; iN = iN + 1)
          if ((!mrgrant[iN] || mrempty[iN]) && (rrequest[iN][M] && !rrequested[iN][M]))
    ...
    for (M = NS; M < NSFULL; M = M + 1) begin : g_srindex
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin : SGRANT
      ...
      always @(*) begin
        swgrant[M] = 0;
        for (iN = 0; iN < NM; iN = iN + 1) if (wgrant[iN][M]) swgrant[M] = 1;
      end
      ...
      always @(*) begin
        srgrant[M] = 0;
        for (iN = 0; iN < NM; iN = iN + 1) if (rgrant[iN][M]) srgrant[M] = 1;
      end
      // }}}
    end
  endgenerate
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin : W5_WRITE_SLAVE_OUTPUTS
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN || !swgrant[M]) axi_awvalid <= 0;
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN || !swgrant[M]) axi_wvalid <= 0;
        else if (!swstall) begin
          axi_wvalid <= (m_wvalid[swindex[M]]) && (slave_waccepts[swindex[M]]);
        end
      ...
      //
      always @(*)
        if (!swgrant[M]) axi_bready = 1;
        else axi_bready = bskd_ready[swindex[M]];
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin : R5_READ_SLAVE_OUTPUTS
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN || !srgrant[M]) axi_arvalid <= 0;
        else if (!arstall)
          axi_arvalid <= m_arvalid[srindex[M]] && slave_raccepts[srindex[M]];
        else if (M_AXI_ARREADY[M]) axi_arvalid <= 0;
      ...
      always @(posedge S_AXI_ACLK)
        if (OPT_LOWPOWER && (!S_AXI_ARESETN || !srgrant[M])) begin
          ...
          if (!OPT_LOWPOWER ||
              (m_arvalid[srindex[M]] && slave_raccepts[srindex[M]])) begin
      ...
      always @(*)
        if (!srgrant[M]) axi_rready = 1;
        else axi_rready = rskd_ready[srindex[M]];
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : W6_WRITE_RETURN_CHANNEL
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) berr_valid[N] <= 0;
        else if (wgrant[N][NS] && m_wvalid[N] && m_wlast[N] && slave_waccepts[N])
          berr_valid[N] <= 1;
        else if (bskd_ready[N]) berr_valid[N] <= 0;

      always @(*)
        if (berr_valid[N]) bskd_valid[N] = 1;
        else bskd_valid[N] = mwgrant[N] && m_axi_bvalid[mwindex[N]];
  ...
  // Return values
  generate
    for (N = 0; N < NM; N = N + 1) begin : R6_READ_RETURN_CHANNEL
      // {{{
      ...
      always @(*)
        if (rgrant[N][NS]) rskd_valid[N] = !rerr_none[N];
        else rskd_valid[N] = mrgrant[N] && m_axi_rvalid[mrindex[N]];
      ...
      // rskid, the outgoing read skidbuf
      // {{{
      ...
      skidbuf #(
        ...
        S_AXI_RVALID[N],
        S_AXI_RREADY[N],
        {S_AXI_RID[N*IW+:IW], S_AXI_RDATA[N*DW+:DW], S_AXI_RLAST[N], S_AXI_RRESP[N*2+:2]}
        // }}}
      );
      // }}}
      // }}}
    ...
    for (N = 0; N < NM; N = N + 1) begin : W7_COUNT_PENDING_WRITES
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) begin
          ...
          case ({
            (m_awvalid[N] && slave_awaccepts[N]), (bskd_valid[N] && bskd_ready[N])
          })
            2'b01: begin
    ...
    for (N = 0; N < NM; N = N + 1) begin : R7_COUNT_PENDING_READS
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) begin
          ...
          case ({
            (m_arvalid[N] && slave_raccepts[N] && !rgrant[N][NS]),
            (rskd_valid[N] && rskd_ready[N] && rskd_rlast[N] && !rgrant[N][NS])
          })
            2'b01: begin
              rpending   <= rpending - 1;
      ...
      always @(posedge S_AXI_ACLK)
        ...
        end else if (m_arvalid[N] && rrequest[N][NS] && slave_raccepts[N]) begin
          rerr_none[N]        <= 0;
      ...
      initial rerr_id[N] = 0;
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN && OPT_LOWPOWER) rerr_id[N] <= 0;
        else if (m_arvalid[N] && slave_raccepts[N]) begin
          if (rrequest[N][NS] || !OPT_LOWPOWER)
            // A low-logic definition
            rerr_id[N] <= m_arid[N];
          else rerr_id[N] <= 0;
        end else if (OPT_LOWPOWER && rerr_last[N] && (!rskd_valid[N] || rskd_ready[N]))
          rerr_id[N] <= 0;
      // }}}
      // }}}
    end
