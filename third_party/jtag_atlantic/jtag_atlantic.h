 ...
 */
struct JTAGATLANTIC *jtagatlantic_open(char const *cable, int device, int instance, char const *progname);
 ...
 */
void jtagatlantic_get_info(struct JTAGATLANTIC *atlantic, char const **cable, int *device, int *instance);
 ...
 */
int jtagatlantic_cable_warning(struct JTAGATLANTIC *atlantic);
 ...
 */
int jtagatlantic_read(struct JTAGATLANTIC *atlantic, char *data, unsigned int len);
 ...
 */
int jtagatlantic_write(struct JTAGATLANTIC *atlantic, char const *data, unsigned int len);

/* jtagatlantic_close: Close the UART. */
void jtagatlantic_close(struct JTAGATLANTIC *atlantic);

/* jtagatlantic_flush: Wait for data to be flushed from the send buffer. */
int jtagatlantic_flush(struct JTAGATLANTIC *atlantic);

/* jtagatlantic_is_setup_done: Return non-zero if UART setup is done. */
int jtagatlantic_is_setup_done(struct JTAGATLANTIC*);

/* jtagatlantic_wait_open: Wait for UART setup to be done. */
int jtagatlantic_wait_open(struct JTAGATLANTIC *atlantic);

/* jtagatlantic_bytes_available: Return number of bytes available for reading */
int jtagatlantic_bytes_available(struct JTAGATLANTIC *atlantic);
